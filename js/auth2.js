...
create.addEventListener('submit', function (e) {
    ...
    //     // .then(result => { result.addEventListener('load', function () {
    //     //     //     if (request.readyState === 4 && request.status == 200) { let data = JSON.parse();
    ...
    M.Modal.getInstance(create).close();
    createForm.reset();
}); //END of CREATE
...
createWish.addEventListener('submit', function (e) {
    ...
    const data = {
        // titleAdds: wishForm['wish_title'].value
        titleAdds: e.target.elements.wishTitle.value,
        content: e.target.elements.problem_content.value
    }
    console.log(data);
    fetch(`https://rest-api20.firebaseio.com/user_list/${localStorage.getItem('localId')}/wishes.json`, {
            method: 'POST',
